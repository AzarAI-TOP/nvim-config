-- Code formatting (conform.nvim) — lazy loaded on <leader>lf (its only entry
-- point; there is deliberately no format-on-save), via the preload stub on
-- the conform module.
--
-- conform does not install formatters — they must be on PATH (Mason's job).
-- Missing formatters are skipped silently per filetype.

require("config.lazy").defer("conform", {
    mods = { "conform" },
    loader = function()
        vim.pack.add({
