-- Buffer management (bento.nvim, v2 API). The `;` key opens the floating
-- buffer switcher; v2 registers no keys or actions by default, so the v1 key
-- surface is rebuilt below.

vim.pack.add({
    -- v2 lives on feat/v2: main is still v1 (the announced July 2026 merge has
    -- not happened; the repo has been dormant since 2026-05). The lock file
    -- pins the exact rev, so branch drift only enters through a deliberate
    -- :PackUpdate.
...
-- Same key for both: menu closed -> jump to the last buffer, menu open ->
-- expand (upstream's recipe for the v1 main_keymap).
